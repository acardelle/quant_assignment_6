...
```
For examining interactions amongst the commuting patterns, we decided to interact each commuting type with the commuting by walking (pct_walk) variable. Additionally, we tested our a categorical variable (white_majority) with commuting by walking for any possible effects. To our surprise, all of our interactions resulted in significant observations with increased correlation values as measured by R2.

For further analysis, we decided to focus on the interaction between commuting by walking and commuting by car since it provided the best model fit (0.47)
